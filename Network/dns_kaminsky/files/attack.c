...
void send_dns_response(unsigned char *packet, int pkt_size)
{
  ...
  for (unsigned short id = 0; id < 65535; id++) {
    ...
    unsigned short id_net_order = htons(id); //Converts the transaction ID to network byte order
    memcpy(packet+28, &id_net_order, 2); //Sets the transaction ID field (at offset 28) in the packet
